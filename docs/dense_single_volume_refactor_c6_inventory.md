...
| working tree | Grouped the exact-local score diagnostic tensors into `LocalScoreDumpCapture`; the writer now accepts eight routing arguments instead of 26 while preserving the existing payload and artifact schema. |
...
| Local score diagnostic capture slice | `tests/unit/test_local_diagnostics.py tests/unit/test_refine_relion_mode.py -k local_score_debug_dump_records_attempted_pose_metadata -q`: 4 passed including the routing and artifact-schema cases; structural score is 31 package long signatures and 2 C6-core long signatures. |
