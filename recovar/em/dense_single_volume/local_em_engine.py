...
from recovar.em.dense_single_volume.local_projection_cache import (  # noqa: F401
    EXACT_LOCAL_RELION_PROJECTION_CACHE_MAX_GB,
    EXACT_LOCAL_RELION_PROJECTION_CACHE_MAX_GB_ENV,
    EXACT_LOCAL_RELION_PROJECTION_CACHE_MAX_GROUPS,
    ...
    _exact_local_relion_projection_cache_capacity_rows,
    _exact_local_relion_projection_cache_chunk_rows,
    _exact_local_relion_projection_cache_max_groups,
    _plan_exact_local_relion_projection_cache_groups,
    _sort_buckets_for_relion_projection_cache,
...
def _exact_local_bpref_contribution_capture_for_call(
    *,
    ...
    if not mstep_relion_x_half:
        raise RuntimeError("Exact-local BPref contribution capture requires RELION x-half M-step geometry")
    return True
...
def _postprocess_local_bucket(
    *,
    ...
    n_significant_samples_np = np.asarray(n_significant_samples, dtype=np.int32) if collect_significant_counts else None
...
def _prepare_local_exact_bucket(
    ...
    ctf_t0 = time.time()
    ctf_eval_params = jnp.asarray(ctf_params, dtype=score_real_dtype) if score_real_dtype is not None else ctf_params
...
def run_local_em_exact(
    ...
    *,
    ...
    geometry_plan = plan_local_em_geometry(
        experiment_dataset=local_request.inputs.experiment_dataset,
        local_layout=local_request.inputs.local_layout,
        search=local_request.search,
    )
    ...
    input_plan = plan_local_em_inputs(
        local_layout=local_request.inputs.local_layout,
        corrections=local_request.corrections,
        posterior=local_request.posterior,
        search=local_request.search,
    )
    ...
    defer_local_noise_projection = (
        not require_materialized_recon_projection and window_spec.use_window and can_defer_local_noise_projection
    )
    ...
    exact_local_progress_seconds = (
        DEFAULT_EXACT_LOCAL_PROGRESS_SECONDS if progress_seconds_override is None else int(progress_seconds_override)
    )
    ...
    relion_score_translation_angles = _sparse_pass2_diagnostics._relion_cuda_score_translation_angles_if_available(
        local_layout.translation_grid,
        image_shape,
    ...
    relion_projection_cache_stats = LocalRelionProjectionCacheStats()
    if use_relion_projector:
        ...
        if compact_relion_projector_big_jit:
            big_jit_relion_projector_output_size = int(2 * window_spec.max_r) if window_spec.max_r is not None else 0
            if score_only:
    ...
    relion_projection_cache_plan = plan_local_relion_projection_cache(
        bucket_specs,
        local_layout.rotation_ids_flat,
        n_projection_pixels=int(window_spec.n_projection),
        enabled=bool(use_big_jit_buckets and compact_relion_projector_big_jit and not score_only),
    )
    bucket_specs = list(relion_projection_cache_plan.buckets)
    ...
    for bucket_index, bucket in enumerate(bucket_specs):
        if (
            relion_projection_cache_plan.groups
            and relion_projection_cache_group_cursor < len(relion_projection_cache_plan.groups)
            and bucket_index == relion_projection_cache_plan.groups[relion_projection_cache_group_cursor][0]
        ):
            group_start, group_stop, _ = relion_projection_cache_plan.groups[relion_projection_cache_group_cursor]
            relion_projection_cache = _build_exact_local_relion_projection_cache_for_buckets(
                ...
                n_projection_pixels=relion_projection_cache_plan.n_projection_pixels,
                ...
                cache_row_capacity=relion_projection_cache_plan.capacity_rows,
                max_global_rotation_id=max(relion_projection_cache_plan.id_map_rows - 1, 0),
                group_index=relion_projection_cache_group_cursor,
                n_groups=len(relion_projection_cache_plan.groups),
            )
            relion_projection_cache_group_cursor += 1
            relion_projection_cache_stats.record(relion_projection_cache)
        n_chunks += 1
        if collect_profile_stats:
        ...
        need_local_recon_projection_for_bucket = bool(
            need_local_recon_projection or (debug_score_dump_bucket_matches and debug_score_dump_operands)
        )
        ...
        if use_big_jit_buckets_for_bucket and significant_backprojection_candidate:
            sparse_big_jit_mstep_estimated_gb, sparse_big_jit_mstep_cap_gb = _sparse_big_jit_mstep_tensors_memory_gb(
        ...
        deferred_big_jit_backprojection = can_defer_big_jit_backprojection and (
            force_deferred_big_jit_backprojection or not sparse_big_jit_backprojection
        )
        if (
        ...
        if use_big_jit_buckets_for_bucket and (
            ...
            sample_mask_arg = None if bucket.local_sample_mask is None else jnp.asarray(bucket.local_sample_mask)
            ...
            return_big_jit_deferred_mstep_inputs = deferred_big_jit_backprojection and not return_big_jit_mstep_tensors
            ...
            raw_big_jit_result = run_local_bucket_big_jit(
                LocalBigJitImageInputs(
                    ...
                    integer_pre_shifts=integer_pre_shifts_arg,
                    fourier_pre_shifts=fourier_pre_shifts_arg,
                    image_corrections=image_corrections_arg,
                    image_only_corrections=image_only_corrections_arg,
                    scale_corrections=scale_corrections_arg,
                    translation_sqdist_ang=translation_sqdist_arg,
                    group_ids=group_ids_arg,
                ),
                LocalBigJitProjectionInputs(
                    mean=mean_for_proj_big_jit,
                    relion_projector_half=relion_projector_half_big_jit,
                    pixel_indices=big_jit_projection_pixel_indices_arg,
                    score_take_indices=big_jit_projection_score_take_arg,
                    reconstruction_take_indices=big_jit_projection_recon_take_arg,
                    cache=relion_projection_cache.projections,
                    cache_id_map=relion_projection_cache.id_map,
                ),
                LocalBigJitMstepAccumulators(y=Ft_y, ctf=Ft_ctf),
                LocalBigJitNoiseAccumulators(
                    wsum=noise_wsum_arg,
                    image_power=noise_img_power_arg,
                    a2=noise_a2_arg,
                    xa=noise_xa_arg,
                    scale_xa=noise_scale_xa_arg,
                    scale_aa=noise_scale_aa_arg,
                    sigma2_offset=noise_sigma2_offset,
                    sumw=noise_sumw,
                ),
                LocalBigJitSharedInputs(
                    image_mask=big_jit_image_mask_arg,
                    ...
                    norm_half_weights=norm_half_weights,
                    score_window_indices=big_jit_window_indices_arg,
                    reconstruction_window_indices=big_jit_recon_window_indices_arg,
                    mstep_reconstruction_window_indices=big_jit_mstep_recon_window_indices_arg,
                    shell_indices_half=shell_indices_half_arg,
                    shell_indices_noise=shell_indices_noise_arg,
                    noise_variance_for_noise=noise_variance_for_noise_arg,
                    scale_correction_pixel_mask=scale_correction_pixel_mask_arg,
                ),
                LocalBigJitBucketInputs(
                    rotation_ids=jnp.asarray(bucket.local_rotation_ids, dtype=jnp.int32),
                    rotations=jnp.asarray(bucket.local_rotations),
                    mstep_rotations=jnp.asarray(_local_mstep_rotations(bucket)),
                    rotation_log_prior=local_rotation_log_prior_arg,
                    translation_log_prior=jnp.asarray(bucket.translation_log_prior),
                    rotation_mask=jnp.asarray(bucket.local_rotation_mask),
                    sample_mask=sample_mask_arg,
                    valid_image_mask=jnp.asarray(valid_image_mask),
                    normalization_log_z=normalization_log_z_arg,
                    normalization_log_evidence=normalization_log_evidence_arg,
            ...
            if return_big_jit_debug_arrays:
                ...
                debug_scores_unpadded = debug_scores[:unpadded_batch_size] if return_big_jit_debug_scores else None
            ...
            if return_big_jit_deferred_mstep_inputs and accumulate_noise:
                ...
                bucket_group_ids = group_ids_arg[:unpadded_batch_size] if group_ids_np is not None else None
                ...
                for chunk_start in range(0, packed_rotation_count, chunk_rows):
                    ...
                    if noise_scale_xa is not None:
                        ...
                        noise_scale_xa = noise_scale_xa.at[bucket_group_ids].add(
                            scale_xa_per_image.astype(noise_scale_xa.dtype)
                        )
                        noise_scale_aa = noise_scale_aa.at[bucket_group_ids].add(
                            scale_aa_per_image.astype(noise_scale_aa.dtype)
                        )
                if return_profile:
            ...
            significant_sample_count, reconstruction_row_count = _postprocess_local_bucket(
                ...
                probs_sum_t=(
                    stats_probs_sum_t[:unpadded_batch_size] if stats_probs_sum_t_np is None else stats_probs_sum_t_np
                ),
        ...
        threshold_for_bucket = (
            ...
            if reconstruction_probability_threshold_np is None
                ...
                reconstruction_probability_threshold_np[np.asarray(bucket.image_indices)], dtype=jnp.float64
            )
        )
        ...
        if accumulate_noise:
            ...
            if translation_sqdist_ang is not None:
                ...
                noise_sumw_offset = jnp.sum(
                    ...
                    * jnp.asarray(translation_sqdist_ang, dtype=precision_policy.score_real_dtype),
                )
            else:
    ...
    profile_summary = {
        ...
        "relion_projection_cache_enabled": np.asarray(relion_projection_cache_stats.groups_built > 0),
        "relion_projection_cache_groups": np.int64(len(relion_projection_cache_plan.groups)),
        "relion_projection_cache_groups_built": np.int64(relion_projection_cache_stats.groups_built),
        "relion_projection_cache_rows": np.int64(relion_projection_cache_stats.max_rows),
        "relion_projection_cache_capacity_rows": np.int64(relion_projection_cache_plan.capacity_rows),
        "relion_projection_cache_id_map_rows": np.int64(relion_projection_cache_plan.id_map_rows),
        "relion_projection_cache_pixels": np.int64(relion_projection_cache_plan.n_projection_pixels),
        "relion_projection_cache_estimated_gb": np.float64(relion_projection_cache_stats.max_estimated_gb),
        "relion_projection_cache_build_s": np.float64(relion_projection_cache_stats.total_build_s),
        "relion_projection_cache_cap_gb": np.float64(relion_projection_cache_plan.cap_gb),
