"""RELION parity sink: per-iteration observables for diff comparison.
...
class ParityDiagnostics:
    """Typed adapter around the stable parity and timing artifact schema.

    The detailed capture calls remain named because they carry schema-specific
    values that do not belong in the generic lifecycle events.  Lifecycle
    methods are observational and never return an alternate algorithm value.
    """
    ...
    def timing_is_active(self) -> bool:
        return timing_is_active()

    def start_iteration(self, iteration: int) -> None:
        start_iteration(iteration)

    def mark_stage(self, iteration: int, stage: str) -> None:
        mark_stage(iteration, stage)

    def collect_e_step(self, **payload) -> None:
        collect_e_step(**payload)

    def dump_iteration(self, **payload) -> None:
        dump_iteration(**payload)

    def dump_timing_iteration(self, **payload) -> None:
        dump_timing_iteration(**payload)
